...
public class DefaultAlipayServiceImpl implements AlipayServiceAPI {
    ...
    public AlipayResultVO getOrderStatus(String orderId) {
        ...
        log.info("DefaultAlipayServiceAPI - getOrderStatus - userId= " + userId);
    ...
    public boolean trade_query(String orderId) {
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 当订单支付成功 修改订单状态为1
                flag = orderServiceAPI.paySuccess(outTradeNo);
                break;
    ...
    public String trade_precreate(String orderId) {
        String filePath = "";
        // 获取订单信息
        OrderVO orderInfo = orderServiceAPI.getOrderInfoById(orderId);
        ...
        String body = "购买电影票花费" + orderInfo.getOrderPrice();

        // 商户操作员编号，添加此参数可以为商户操作员做销售统计
        String operatorId = "hanjw";
...
//                ftpUtil.setUploadPath("/qrcode");
                ...
                boolean isSucess = ftpUtil.uploadFile(fileName, qrCodeImge);
                if (!isSucess) {
                    filePath = "";
